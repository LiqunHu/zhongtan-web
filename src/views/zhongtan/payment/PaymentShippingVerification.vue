    ...
    <panel title="Shipping Manager">
          ...
          <div class="form-inline">
            ...
            <div class="form-group m-r-2">
              <Select v-model="search_data.verification_state" style="width:180px" @on-change="getPaymentVerificationData">
                <Option v-for="item in pagePara.PAYMENT_STATE" :value="item.id" :key="item.id">{{ item.text }}</Option>
              </Select>
            </div>
      ...
      <Table stripe ref="paymentVerification" :columns="table.paymentVerification.rows" :data="table.paymentVerification.data" :border="table.paymentVerification.data && table.paymentVerification.data.length > 0">
        <template slot-scope="{ row, index }" slot="action">
          <a v-if = "row.payment_verification_state == 'PSM'" href="#" class="btn btn-primary btn-icon btn-sm" @click.stop="approve(row)">
            <i class="fa fa-check"></i>
          </a>
          <a v-if = "row.payment_verification_state == 'PSM'" href="#" class="btn btn-danger btn-icon btn-sm" @click.stop="decline(row)">
...
const apiUrl = '/api/zhongtan/payment/ShippingManager/'

export default {
  name: 'PaymentShippingVerification',
  data: function() {
    return {
        ...
        paymentVerification: {
          rows: [
            ...
            },
            {
              ...
              width: 180,
              render: common.selectRender(this, 'PAYMENT_STATE'),
            },
            {

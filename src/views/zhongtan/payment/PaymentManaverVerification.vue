    ...
    <panel title="Manager Verification">
      ...
      <Table stripe ref="paymentVerification" :columns="table.paymentVerification.rows" :data="table.paymentVerification.data" :border="table.paymentVerification.data && table.paymentVerification.data.length > 0">
        <template slot-scope="{ row, index }" slot="action">
          <a v-if = "row.payment_verification_state == 'PB' || row.payment_verification_state == 'PM'" href="#" class="btn btn-primary btn-icon btn-sm" @click.stop="approve(row)">
            <i class="fa fa-check"></i>
          </a>
          <a v-if = "row.payment_verification_state == 'PB' || row.payment_verification_state == 'PM'" href="#" class="btn btn-danger btn-icon btn-sm" @click.stop="decline(row)">
        ...
        paymentVerification: {
          rows: [
            ...
            },
            {
              ...
              render: common.selectRender(this, 'PAYMENT_VERIFICATION_STATE'),
            },
            {

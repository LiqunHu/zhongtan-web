      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-info" v-on:click="resetOp"><i class="fa fa-fw fa-plus"></i>提交
            </button>
          </div>
        </div>
      </div>
...
const apiUrl = '/api/common/system/ResetPassword/'
export default {
  data: function() {
    return {
      pagePara: {},
  ...
  mounted: function() {
    ...
    function initPage() {
      _self.$http.post(apiUrl + 'search', {}).then(
        response => {
  ...
  methods: {
    resetOn: function(event) {
      let _self = this
      if ($('#username').val()) $('#ResetModal').modal('show')

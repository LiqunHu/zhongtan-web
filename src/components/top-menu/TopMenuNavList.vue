<template>
  ...
  <li v-if="menu.sub_menu" class="has-sub" v-bind:class="{ 'active': subIsActive(menu.menu_path) }">
    ...
    >
      <template v-for="submenu in menu.sub_menu">
        <top-menu-nav-list v-bind:menu="submenu" v-bind:key="submenu.menu_name" ref="topMenuNavList" v-on:collapse-other="handleCollapseOther(submenu)"></top-menu-nav-list>
      </template>
    </ul>
  </li>
  ...
  <router-link v-else v-bind:to="menu.menu_path" active-class="active" tag="li" v-on:click.native="collapseOther()">
    <a>
      <span v-if="menu.badge" class="badge pull-right">{{ menu.badge }}</span>
      <i v-if="menu.icon" v-bind:class="'fa ' + menu.menu_icon"></i>
      <div v-if="menu.img" class="icon-img">
        <img v-bind:src="menu.img" alt>
      </div>
      <span>{{ menu.menu_name }}</span>
...
export default {
  ...
  methods: {
    ...
    handleCollapseOther: function(menu) {
      for (var i = 0; i < this.menu.sub_menu.length; i++) {
